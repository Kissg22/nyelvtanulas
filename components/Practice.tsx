...
export default function Practice(){
  ...
  useEffect(()=>{fetch('/api/practice').then(r=>r.json()).then(d=>{setWords(shuffled(d.words||[]));setLoading(false)})},[]);
  ...
  useEffect(()=>{if(type==='listening'&&current){const id=setTimeout(speak,250);return()=>clearTimeout(id)}},[type,current]);
  function record(correct:boolean){setFeedback(correct?'correct':'wrong');if(correct)setScore(s=>s+1);setTimeout(()=>{setFeedback('');setAnswer('');setIndex(i=>i+1)},900)}
  ...
  if(index>=words.length)return <div className="card practice-card"><h2>Kör kész</h2><div className="practice-prompt">{score} / {words.length}</div><p className="muted">Az eredmény csak erre a körre vonatkozik, nem tárolunk külön gyakorlási előzményeket.</p><button className="button" onClick={()=>location.reload()}>Új kör</button></div>;
